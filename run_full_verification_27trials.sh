...
echo "参数: 20000数据, 27trials, 3epochs, GPU:$GPU_LIST"
echo "=========================================="
...
for MODE in "${MODES[@]}"; do
    ...
    if [ "$MODE" = "serial" ]; then
        ...
        EXTRA_ARGS="--data-parallel-workers 3 --gpu-list $GPU_LIST"
    elif [ "$MODE" = "pipeline_naive" ]; then
        EXEC_MODE="ray_pipeline"
        EXTRA_ARGS="--pipeline-mode naive --gpu-list $GPU_LIST"
    elif [ "$MODE" = "pipeline_smart" ]; then
        EXEC_MODE="ray_pipeline"
        EXTRA_ARGS="--pipeline-mode smart --gpu-list $GPU_LIST"
    fi

    # 第1步：NAS搜索
    echo "[1/2] 运行NAS搜索 (27 trials, 3 epochs)..."
    python search.py \
        --search-mode rl \
...
print(f"\n{'='*80}")
print(f"NAS vs Retrain 对比报告 (Seed={SEED}, 27trials, 3epochs)")
print(f"{'='*80}\n")

print(f"{'模式':<15} {'NAS MRR':<10} {'Retrain MRR':<12} {'差异':<10} {'Seed':<10} {'参数':<20}")
print("-" * 80)
...
for mode in modes:
    ...
    if not os.path.exists(best_arch_path):
        print(f"{mode:<15} {'N/A':<10} {'N/A':<12} {'N/A':<10} {'N/A':<10} {'N/A':<20}")
        continue
    ...
    # 关键参数
    params = f"norm={config.get('normalize_state','?')},static={config.get('use_static_embeddings','?')}"
    ...
    print(f"{mode:<15} {nas_mrr:<10.4f} {retrain_mrr:<12.4f} {diff_pct:<10} {str(nas_seed):<10} {params:<20}")

print("\n" + "="*80)
print("验证所有修复:")
print("  ✓ normalize_state参数提取")
print("  ✓ use_static_embeddings参数提取")
print("  ✓ Pipeline模式seed保存")
print("  ✓ 种子设置时机")
print("  ✓ 设备设置")
EOFPYTHON
