...
def split_partition_interactions(
    ...
    """将 partition.interactions 按时间顺序分割成 num_workers 个块。"""
    interactions = partition.interactions
    if num_workers <= 1:
        return [list(interactions)]

    # 按时间范围而非数量分割，以减少用户/物品重叠
    if not interactions:
    ...
    for inter in interactions:
        ...
        worker_id = min(worker_id, num_workers - 1)  # 确保最后一个块获取边界事件
...
def _build_count_partitions(interactions: List[Interaction], partition_size: int, overlap_ratio: float = 0.0) -> List[List[Interaction]]:
    ...
    # 防止当 overlap_ratio >= 1.0 或接近 1 时 step == 0，这将导致无限循环
    if step == 0:
        raise ValueError(
            f"partition_size * (1 - overlap_ratio) evaluates to 0: "
            f"partition_size={partition_size}, overlap_ratio={overlap_ratio}. "
            f"Reduce overlap_ratio or increase partition_size."
        )
    partitions = []
    start = 0
