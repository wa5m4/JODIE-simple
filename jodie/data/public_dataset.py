"""加载并规范化公开的 JODIE 风格数据集，转换为 Interaction 事件。"""
...
def _resolve_dataset_path(dataset_name: str, dataset_dir: str, local_data_path: str) -> str:
    # BUG 修复："public_csv" 不在 _JODIE_URLS 中，因此在 URL 查找之前处理它
    if dataset_name == "public_csv":
        if not local_data_path:
            raise ValueError("dataset=public_csv requires a local_data_path")
