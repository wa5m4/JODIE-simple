...
def _aggregate_stage_costs(partition_costs: List[float], num_stages: int) -> List[float]:
    """将分区级成本聚合为阶段级成本（按分区数量均匀分组后求和）。"""
    if not partition_costs or num_stages <= 0:
        return [1.0]
    n = len(partition_costs)
    base = n // num_stages
    remainder = n % num_stages
    ...
    for i in range(num_stages):
        ...
        stage_costs.append(max(1.0, sum(partition_costs[start:end])))
        start = end
    return stage_costs
...
class ConfigOptimizer:
    ...
    def auto_allocate_config_advanced(
        ...
        2. worker：基于 stage 成本的最优分配（若提供 partition_costs 则按成本加权）
        ...
        # 步骤 1：stage 数（基于 events_per_gpu）
        ...
        # 步骤 2：基于成本的 worker 分配（若提供 partition_costs 则按成本加权）
        if partition_costs and len(partition_costs) > 0:
            stage_costs = _aggregate_stage_costs(partition_costs, S)
            workers = ConfigOptimizer._optimal_worker_allocation(stage_costs, m)
            info_lines.append(f"Workers (cost-weighted): {workers} (stage_costs={[f'{c:.0f}' for c in stage_costs]})")
        else:
