...
from jodie.data.synthetic import generate_synthetic_data, init_dynamic_graph_state
...
from .data_parallel import DataParallelExecutor
from .ray_pipeline import RayPipelineExecutor
from .search_space import canonical_config_signature, sanitize_config
...
class GraphNASTrainer:
        ...
        try:
            ...
            if trace_file:
                ...
                try:
                    ...
                    print(f"[Efficiency Monitor] Failed to generate report: {e}", flush=True)
        except Exception as e:
            print(f"[Efficiency Monitor] Error stopping monitor: {e}", flush=True)
    ...
    def _family_balanced_candidates(
        ...
        """按模型族均衡选择重排序候选，使用 config 签名去重而非 id()。

        使用 canonical_config_signature 作为去重键，避免 Python id()
        在对象重建或 GC 后可能重复的问题。
        """
        selected: List[Dict] = []
        ...
        for family in model_families:
            ...
            for row in family_rows[: max(0, min_per_model)]:
                sig = canonical_config_signature(row["config"])
    ...
    def _search_pipeline_async(
        ...
        pipeline_executor.start_persistent_pool(eval_kwargs, num_train_epochs=coarse_epochs)
        ...
        while len(results) < coarse_trials:
            ...
            for r in completed:
                tid = r["trial_id"]
                stored_logprob = pending_logprobs.pop(tid, None)
                results.append(r)
                cumulative_best = max(cumulative_best, r["score"])
                update_buffer.append((r["config"], r["score"], stored_logprob))
    ...
    def search_pipeline(
        ...
        if self.base_config.get("enable_efficiency_monitor", False) and self.base_config.get("pipeline_trace_log_path"):
            ...
            try:
                ...
                # 在任意退出时自动停止监控（崩溃或正常退出）
                atexit.register(lambda: (
                    self._cleanup_monitor()
                ) if self._monitor_process is not None and self._monitor_process.poll() is None else None)
            except Exception as e:
                print(f"[Efficiency Monitor] Failed to start monitor: {e}", flush=True)
                self._monitor_process = None
        ...
        if self.base_config.get("enable_auto_pipeline_config", False) and partition_plan:
            try:
                from .config_optimizer import ConfigOptimizer, CostModel
                ...
                print(f"\n[Auto-Config Phase 2] DP 优化失败: {e}", flush=True)

        # 为整个搜索创建一次 pipeline executor（跨批次复用）
        ...
        # 在测试集上做最终评估（在 train+val 上训练，在 test 上评估）—— 匹配 JODIE 论文协议
        if self._time_budget_reached(search_start_time, time_budget_sec):
        ...
        final_seed = int(self.base_config.get("seed", 42)) + self.FINAL_RETRAIN_SEED_OFFSET
        ...
        selected["seed"] = int(self.base_config.get("seed", 42)) + self.FINAL_RETRAIN_SEED_OFFSET
        # score/mrr/recall_at_k 保留为验证集分数以确保 NAS 比较的公平性
    ...
    def search_data_parallel(
        ...
        for trial_idx in range(coarse_trials):
            if time_budget_sec > 0 and (time.time() - search_start_time) >= time_budget_sec:
                ...
                break
            # 先采样再评估最后更新 RL —— 与串行搜索顺序相同，以避免
            # 采样和反向传播之间 controller 计算图的原地修改。
            arch, logprob = self._sample_unique_arch(
                controller=controller,
                seen_signatures=seen_signatures,
            )
        ...
        # 杀死 Ray worker 以便 pipeline 拥有全部资源
        executor.shutdown()
